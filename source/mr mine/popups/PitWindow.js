class PitWindow extends TabbedPopupWindow {
    ...
    paymentAmount;
    ownedRelics;
    relicAnimationsCache = [];
    relicRenderContext = null;
    ...
    constructor(boundingBox) {
        ...
        this.ownedRelics = generateOwnedRelicsArray();
        this.sacrificePane.render = function (parent) {
            ...
            if (typeof (root.selectedPaymentIndex) !== 'undefined' && root.selectedPaymentIndex !== null) {
                if (root.currentTabIndex == 0) {
                    ...
                    sacrificeName = excavationRewards[parent.ownedRelics[root.selectedPaymentIndex]].name;
                }
    ...
    render() {
        ...
        if (this.currentTabIndex == 1 && this.relicRenderContext != null) {
            this.renderPitRelics();
        }
        super.render();
    }
    ...
    generateMenuContents(tabIndex, scrollbox) {
        ...
        if (tabIndex >= 0) {
            ...
            if (this.currentTabIndex == 0) {
                ...
                this.relicRenderContext = scrollbox.context;
                this.relicAnimationsCache = [];
            ...
            for (var i = 0; i < itemsInCategory; ++i) {
                ...
                if (this.currentTabIndex == 0) {
                    ...
                    sacrificeName = excavationRewards[this.ownedRelics[i]].name;
                    sacrificeDescription = getRewardDescription(this.ownedRelics[i]);
                    icon = excavationRewards[this.ownedRelics[i]].image;
                    var relicRenderFunction = excavationRewards[this.ownedRelics[i]].renderFunction;
                    var cacheAnimation = {
                        x: slotX,
                        y: slotY,
                        width: slotSize,
                        height: slotSize,
                        relic: this.ownedRelics[i],
                        relicRenderFunction: relicRenderFunction
                    };
                    this.relicAnimationsCache.push(cacheAnimation);
                }
                else if (this.currentTabIndex == 2) {
    ...
    renderPitRelics() {
        this.relicRenderContext.save();
        this.relicRenderContext.clearRect(0, 0, this.paymentPane.boundingBox.width, this.paymentPane.boundingBox.height);
        for (var i = 0; i < this.relicAnimationsCache.length; ++i) {
            this.relicRenderContext.fillStyle = "#000000";
            this.relicRenderContext.globalAlpha = 0.5;
            this.relicRenderContext.fillRect(
                this.relicAnimationsCache[i].x,
                this.relicAnimationsCache[i].y,
                this.relicAnimationsCache[i].width,
                this.relicAnimationsCache[i].height
            );
            this.relicRenderContext.globalAlpha = 1;

            this.relicAnimationsCache[i].relicRenderFunction(this.relicRenderContext, this.relicAnimationsCache[i].relic, this.relicAnimationsCache[i].x, this.relicAnimationsCache[i].y, this.relicAnimationsCache[i].width, this.relicAnimationsCache[i].height);
            drawFrame(
                this.relicRenderContext,
                itemFrame,
                this.relicAnimationsCache[i].x,
                this.relicAnimationsCache[i].y,
                this.relicAnimationsCache[i].width,
            ...
            if (this.selectedPaymentIndex == i) {
                this.relicRenderContext.strokeStyle = "#76E374";
                this.relicRenderContext.lineWidth = 3;
                this.relicRenderContext.beginPath();
                this.relicRenderContext.strokeRect(
                    this.relicAnimationsCache[i].x + this.relicRenderContext.lineWidth,
                    this.relicAnimationsCache[i].y + this.relicRenderContext.lineWidth,
                    this.relicAnimationsCache[i].width - 2 * this.relicRenderContext.lineWidth,
                    this.relicAnimationsCache[i].height - 2 * this.relicRenderContext.lineWidth
                );
                this.relicRenderContext.stroke();
            }
        }
        this.relicRenderContext.restore();
    }
