...
class ActiveDrone extends BaseDrone {
    ...
    startCollectingRewardsOnNode(node) {
        ...
        for (var i = 0; i < rewards.length; ++i) {
            ...
            var canClaim = false;
            if (reward.activateOnPickup) {
                canClaim = reward.canBeCollected(this);
            }
            else {
                canClaim = (inventoryItemsClaimed < this.rewardCapacity - this.inventory.length) &&
        ...
        for (var i in node.children) {
            if (node.children[i] != this.nodePath[this.nextNodeIndex()] &&   // Don't pull rewards from the node we're moving to
                ...
                for (var j in rewardIndices) {
                    ...
                    if (this.caveSystem.rewards[rewardIndices[j]].isClaimed) continue;
                    this.isActing = true;
