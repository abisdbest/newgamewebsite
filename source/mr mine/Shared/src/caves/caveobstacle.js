...
class CaveBlocker extends CaveNode {
    ...
    update(deltaTime, drone) {
        this.currentHealth -= (drone.damagePerSecond * STAT.droneObstacleSpeedMultiplier()) * deltaTime;
        if (this.currentHealth <= 0 && drone.waitAtNodeTime > this.duration) {
            if (drone.id === 1) {
                if (drone.clearingObstacle === true) {
                    drone.totalWaitTime = 0;
                    drone.waitAtNodeTime = 0;
                    drone.clearingObstacle = false;
                    drone.onEnterNode(this);
                }
            }
            else {
                if (!(drone.id === 3 && (drone.lastReachedNodeIndex == drone.nodePath.length - 1))) {
