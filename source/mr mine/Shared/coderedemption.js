...
function onCouponSuccess(coupon) {
	...
	switch (rewardType) {
		...
		case "super_miner_souls":
			...
			rewardMessage = _("You gained {0} Super Miner Souls", rewardValue);
			break;
		case "weapon_scrap":
			rewardValue = parseInt(coupon.value);
			addWeaponScrap(rewardValue);
			rewardMessage = _("You gained {0} Weapon Scrap", rewardValue);
			break;
		case "relic_scrap":
			rewardValue = parseInt(coupon.value);
			addRelicScrap(rewardValue);
			rewardMessage = _("You gained {0} Relic Scrap", rewardValue);
			break;
		case "cheats":
			break;
		default:
...
function onCouponError(error, suppressErrors = false, code = "", retryAttempt = 0) {
	...
	if (error instanceof Playsaurus.CouponRedeemError) { // Redemption logic error
		window.alert(error.serverMessage);
	} else if (error instanceof Playsaurus.PlaysaurusNetworkError) { // Network error
	...
	} else { // Other unknown JavaScript error
