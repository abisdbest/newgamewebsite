...
function mine() {
    ...
    if (!isCapacityFull()) {
            ...
            if (((workersHiredAtDepth(x) * 2) + (workerLevelForDepth * 3)) > rand(0, 39)) {
                for (var y = 0; y < levels[x].length; y++) {
                    ...
                    if (rand(0, 999) < threshold) {
                        var finalMineralIndex = mineralIndex;
                        if (worldResources[mineralIndex].isIsotope) {
                            if (worldResources[mineralIndex].isotopeIndex === 0) {
                                finalMineralIndex = getDecayedIsotopeOneIndex(mineralIndex);
                            }
                            else if (worldResources[mineralIndex].isotopeIndex === 1) {
                                finalMineralIndex = getDecayedIsotopeTwoIndex(mineralIndex);
                            }
                        }
                        worldResources[finalMineralIndex].numOwned++;
                        totalMineralsMined++;
                        mineralsMinedValue = mineralsMinedValue.add(worldResources[finalMineralIndex].sellValue);
                        capacity++;

                        if (managerStructure.level >= 2 && worldResources[finalMineralIndex].numOwned > lockedMineralAmtsToSave[mineralIndex]) {
                            ...
                            if (x <= currentlyViewedDepth && x > currentlyViewedDepth - numberOfDepthsVisible) {
                                if (Math.random() >= percentAttributedToSuperMiner) {
                                    if (!isMobile()) {
                                        ...
                                        found[relativeDepth][0][putw] = finalMineralIndex - 1; //mineral
        ...
        checkForNewMineralDiscoveries();
    }
}

function checkForNewMineralDiscoveries() {
    // Check for new ore discoveries
    ...
    if (numOresDiscovered == 1) {
        ...
        if (!mutebuttons && typeof discoverMineralAudio !== 'undefined') discoverMineralAudio.play();
    }

    // Check for new isotope discoveries
    ...
    if (!isCapacityFull()) {
        ...
        for (var i = 0; i < mineralsPerSecond.length; i++) {
            if (worldResources[i] && !isNaN(mineralsPerSecond[i])) {
                ...
                if (worldResources[i].isIsotope && rewardAmount > 0) {
                    var decayChanceOne = STAT.isotopeOneDecayChancePercent();
                    var decayChanceTwo = STAT.isotopeTwoDecayChancePercent();

                    for (var j = 0; j < rewardAmount; j++) {
                        var currentIndex = i;

                        if (worldResources[i].isotopeIndex === 0 && decayChanceOne > 0) {
                            currentIndex = getDecayedIsotopeOneIndex(i);
                        }
                        else if (worldResources[i].isotopeIndex === 1 && decayChanceTwo > 0) {
                            currentIndex = getDecayedIsotopeTwoIndex(i);
                        }

                        totalMineralsMined += 1;
                        worldResources[currentIndex].numOwned += 1;
                        capacity += 1;
                    }
                }
                else {
...
function applyIsotopeDecayToAmount(mineralIndex, amount) {
    var results = {};
    var isotope = worldResources[mineralIndex];

    if (!isotope.isIsotope) {
        results[mineralIndex] = amount;
        return results;
    }

    var currentIndex = mineralIndex;
    var remainingAmount = amount;

    if (isotope.isotopeIndex === 0) {
        var decayChance = STAT.isotopeOneDecayChancePercent();
        if (decayChance > 0) {
            var decayedAmount = Math.floor(remainingAmount * decayChance);
            var undecayedAmount = remainingAmount - decayedAmount;

            if (undecayedAmount > 0) {
                results[currentIndex] = undecayedAmount;
            }

            if (decayedAmount > 0) {
                var nextIndex = currentIndex + 1;
                if (nextIndex < worldResources.length &&
                    worldResources[nextIndex].isIsotope &&
                    worldResources[nextIndex].isotopeIndex === 1) {
                    var nextLevelResults = applyIsotopeDecayToAmount(nextIndex, decayedAmount);
                    for (var index in nextLevelResults) {
                        results[index] = (results[index] || 0) + nextLevelResults[index];
                    }
                }
                else {
                    results[currentIndex] = (results[currentIndex] || 0) + decayedAmount;
                }
            }
        }
        else {
            results[currentIndex] = remainingAmount;
        }
    }
    else if (isotope.isotopeIndex === 1) {
        var decayChance = STAT.isotopeTwoDecayChancePercent();
        if (decayChance > 0) {
            var decayedAmount = Math.floor(remainingAmount * decayChance);
            var undecayedAmount = remainingAmount - decayedAmount;
            ...
            if (decayedAmount > 0) {
                ...
                if (nextIndex < worldResources.length &&
                    worldResources[nextIndex].isIsotope &&
                    worldResources[nextIndex].isotopeIndex === 2) {
                    results[nextIndex] = decayedAmount;
                }
                else {
                    results[currentIndex] = (results[currentIndex] || 0) + decayedAmount;
                }
...
function sellAllMineralsFromAllPlanets() {
    var moneyPriorToSale = new BigNumber(money);

    for (var i = 0; i < worlds.length; i++) {
        var world = worlds[i];

        for (var j = 0; j < world.mineralIdsToSell.length; j++) {
            sellMineral(world.mineralIdsToSell[j]);
        }

        for (var k = 0; k < world.isotopeIdsToSell.length; k++) {
            sellMineral(world.isotopeIdsToSell[k]);
        }
...
function addSuperMinerSouls(amount) {
    ...
    newNews(_("You gained {0} Super Miner Souls from excavation", amount), true);
}
...
function spendRelicScrap(amount) {
    worldResources[RELIC_SCRAP_INDEX].numOwned -= amount;
    relicScrapSpent += amount;
}
//############################################################
//######################### CAPACITY #########################
//############################################################
...
function lockWorldResources(worldIndex, tab, lockValue) {
    var world = worlds[worldIndex];

    if (tab == 0) {
        for (var i = 0; i < world.mineralIdsToSell.length; i++) {
            var mineralIndex = world.mineralIdsToSell[i];
            if (worldResources[mineralIndex].numOwned > 0) {
                lockedMineralAmtsToSave[mineralIndex] = lockValue;
            }
        }
    }
    else if (tab == 1) {
        for (var j = 0; j < world.isotopeIdsToSell.length; j++) {
            var isotopeIndex = world.isotopeIdsToSell[j];
            if (worldResources[isotopeIndex].numOwned > 0) {
                lockedMineralAmtsToSave[isotopeIndex] = lockValue;
            }
        }
    }
}
...
function getDecayedIsotopeOneIndex(index) {
    let isotope = worldResources[index];
    let decayChance = STAT.isotopeOneDecayChancePercent();

    if (isotope.isotopeIndex === 0 && decayChance > 0 && Math.random() < decayChance) {
...
function getDecayedIsotopeTwoIndex(index) {
    let isotope = worldResources[index];
    let decayChance = STAT.isotopeTwoDecayChancePercent();

    if (isotope.isotopeIndex === 1 && decayChance > 0 && Math.random() < decayChance) {
        let nextIndex = index + 1;
        if (nextIndex < worldResources.length &&
            worldResources[nextIndex].isIsotope &&
            worldResources[nextIndex].isotopeIndex === 2) {
