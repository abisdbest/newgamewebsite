...
window.DOMElementHandler = class extends self.DOMHandler {
  ...
  _OnSetVisible(d) {
    this._autoAttach &&
      (this._elementMap.get(d.elementId).style.display = d.isVisible
        ? ""
        : "none");
  }
  _OnUpdatePosition(d) {
...
("use strict");
{
  const d = /(iphone|ipod|ipad|macos|macintosh|mac os x)/i.test(
    navigator.userAgent
  );
  let a = 0;
  function b(g) {
  ...
  window.RuntimeInterface = class g {
    ...
    async _Init(c) {
        ...
        for (let l = 0, n = c.engineScripts.length; l < n; ++l) {
          ...
          this._localFileStrings.hasOwnProperty(k)
            ...
            : this._localFileBlobs.hasOwnProperty(k) &&
              (c.engineScripts[l] = URL.createObjectURL(
      ...
      this._useWorker
        ? await this._InitWorker(c, k.port2)
        : await this._InitDOM(c, k.port2);
    }
    _GetWorkerURL(c) {
      c = this._workerScriptURLs.hasOwnProperty(c)
        ...
        : "playable-ad" === this._exportType &&
          this._localFileBlobs.hasOwnProperty(c.toLowerCase())
    ...
    async CreateWorker(c, k, l) {
      ...
      if ("cordova" === this._exportType && this._isFileProtocol)
        return (
          (c = await this.CordovaFetchLocalFileAsArrayBuffer(
            l.isC3MainWorker ? c : this._scriptFolder + c
          )),
    ...
    async _InitDOM(c, k) {
      ...
      Array.isArray(c.workerDependencyScripts) &&
        l.unshift(...c.workerDependencyScripts);
      ...
      for (let [q, t] of r)
        if ((t || (t = q), q === n))
          ...
          (t = await this._MaybeGetCordovaScriptURL(t)), await b(t);
      "preview" === this._exportType &&
    ...
    _RemoveLoadingMessage() {
      ...
      c &&
        (c.parentElement.removeChild(c), (window.cr_previewLoadingElem = null));
    }
    ...
    PostToRuntimeComponentAsync(c, k, l, n, r) {
      const q = G++,
        t = new Promise((x, E) => {
    ...
    ["_OnMessageFromRuntime"](c) {
      ...
      if ("result" === k) this._OnResultFromRuntime(c);
        ...
        this._RemoveLoadingMessage(), alert(c.message);
    ...
    async _MaybeGetCordovaScriptURL(c) {
      return "cordova" === this._exportType &&
        (c.startsWith("file:") ||
          (this._isFileProtocol && this.IsRelativeURL(c)))
    ...
    async RequirePermissions(...c) {
      if (!1 === (await this.RequestPermissions(c)))
  ...
  d.AddDOMHandlerClass(
    class extends self.DOMHandler {
      constructor(f) {
        ...
        window.addEventListener("touchstart", y, { passive: !1 });
        "undefined" !== typeof PointerEvent
          ? (window.addEventListener("pointerdown", y, { passive: !1 }),
      ...
      Attach() {
        ...
        window.addEventListener("blur", () => {
          this._PostRuntimeEvent("window-blur", { parentHasFocus: H() });
          this._mousePointerLastButtons = 0;
        });
      ...
      _OnDeviceMotion(f) {
        ...
        l &&
          (k = { alpha: l.alpha || 0, beta: l.beta || 0, gamma: l.gamma || 0 });
        this.PostToRuntime(
          "devicemotion",
          {
            acceleration: g,
      ...
      _OnWrapperMessage(f) {
        "entered-fullscreen" === f
          ? (d._SetWrapperIsFullscreenFlag(!0), this._OnFullscreenChange())
          : "exited-fullscreen" === f
          ? (d._SetWrapperIsFullscreenFlag(!1), this._OnFullscreenChange())
...
self.JobSchedulerDOM = class {
  ...
  async Init() {
    ...
    var d = this._runtimeInterface._GetWorkerURL("");
    ...
    this._dispatchWorker.postMessage({ type: "_init", "in-port": d.port2 }, [
      d.port2,
    ]);
  ...
  async _CreateJobWorker() {
    ...
    var a = this._runtimeInterface._GetWorkerURL("./scripts/jobworker.js");
    a = await this._runtimeInterface.CreateWorker(a, this._baseUrl, {
      name: "JobWorker" + d,
    });
    const b = new MessageChannel(),
      e = new MessageChannel();
    this._dispatchWorker.postMessage({ type: "_addJobWorker", port: b.port1 }, [
      b.port1,
    ]);
    a.postMessage(
      {
...
self.C3WebAudioBuffer = class extends self.C3AudioBuffer {
  ...
  async _Fetch() {
    ...
    if (
      ...
      if (!d.ok)
        throw Error(`error fetching audio data: ${d.status} ${d.statusText}`);
...
self.C3Html5AudioInstance = class extends self.C3AudioInstance {
  ...
  SetLooping(d) {
    d = !!d;
    this._isLooping !== d &&
      ((this._isLooping = d), (this.GetAudioElement().loop = d));
  }
  _UpdatePlaybackRate() {
...
self.C3WebAudioInstance = class extends self.C3AudioInstance {
  ...
  SetLooping(d) {
    ...
    this._isLooping !== d &&
      ((this._isLooping = d),
      this._bufferSource && (this._bufferSource.loop = d));
  }
  _UpdatePlaybackRate() {
    let d = this._playbackRate;
    this._isTimescaled && (d *= this._audioDomHandler.GetTimeScale());
...
self.RuntimeInterface.AddDOMHandlerClass(
  class extends self.DOMHandler {
    constructor(d) {
      super(d, "mouse");
      this.AddRuntimeMessageHandler("cursor", (a) =>
        this._OnChangeCursorStyle(a)
      );
    }
  ...
  class extends self.DOMHandler {
    ...
    async _RequestOrientationPermission() {
      ...
      try {
        ...
        return (
          console.warn("[Touch] Failed to request orientation permission: ", d),
          !1
        );
      }
    }
    async _RequestMotionPermission() {
      ...
      try {
        ...
        return (
          console.warn("[Touch] Failed to request motion permission: ", d), !1
        );
      }
    }
  }
